...
public class ImportExcel {
	 ...
	 * @updated 修改描述.
	 * @updated by sxm
	 * @updated at 2014年2月17日
	 */
	public int getFirstCellNum() {
		return this.getRow(headerNum).getFirstCellNum();
	}
			...
			for (Object[] os : annotationList) {
				...
				if (val != null) {
					...
					if (StringUtils.isNotBlank(ef.dictType())) {
						String className = "com.sxm.core.common.sys.utils.DictUtils";
						String[] args = { val.toString(), ef.dictType(), "" };
						try {
							Class<?> clazz = Class.forName(className);
							val = MethodUtils.invokeStaticMethod(clazz, "getDictValue", args);
						} catch (Exception ex) {
							// Failure to ignore
					...
					try {
						if (valType == String.class) {
							...
							Cell cell = row.getCell(column);
							if (cell.getCellType() == Cell.CELL_TYPE_STRING) { // 当日期类型在excel里是字符类型时
								val = Dates.parse(val.toString());
							} else {
						...
						logger.error("Get cell value [" + i + "," + column + "] error: " + ex.toString());
						val = null;
					}
					// set entity value
					if (os[1] instanceof Field) {
