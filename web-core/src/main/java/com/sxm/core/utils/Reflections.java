...
import com.sxm.core.domain.Pk;
import com.sxm.core.domain.Sys;
import com.sxm.core.domain.Tree;
...
public class Reflections {
	...
	public static Object getFieldValue(final Object obj, final String fieldName) {
		...
		Object result = null;
		try {
			if (Strings.contains(obj.getClass().getSimpleName(), CGLIB_CLASS_SEPARATOR)) {
				result = BeanUtils.getProperty(obj, fieldName);
			} else {
	...
	public static Field[] getDeclaredFields(final Class<?> clazz) {
		Validate.notNull(clazz, "clazz can't be null");
		Class<?> targetClazz = Cglibs.getTarget(clazz);
		List<Field> list = new ArrayList<Field>();
		for (Class<?> superClass = targetClazz; superClass != Object.class; superClass = superClass.getSuperclass()) {
			if (superClass == Pk.class || superClass == Sys.class || superClass == Tree.class) {
				continue;
			}
	 ...
	 */
	public static Method getDeclaredMethod(final Object obj, final String methodName) {
		Validate.notNull(obj, "obj can't be null");
	 ...
	 * 循环向上判断是否是继承关系.
	 */
	public static boolean isAssignableFrom(final Class<?> sub, final Class<?> base) {
		if (base.isAssignableFrom(sub)) {
			return true;
		}
		for (Class<?> superClass = sub.getClass(); superClass != Object.class; superClass = superClass.getSuperclass()) {
			if (base.isAssignableFrom(superClass)) {
				return true;
			}
		}
		return false;
	...
	public static Class<?> getClassGenricType(final Class<?> clazz, final int index) {
		...
		if (!(genType instanceof ParameterizedType)) {
			genType = clazz.getSuperclass().getSuperclass().getGenericSuperclass();
		}
