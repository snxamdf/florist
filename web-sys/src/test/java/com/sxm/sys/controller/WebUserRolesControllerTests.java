...
public class WebUserRolesControllerTests {
	...
	public void list() throws Exception {
		String url = "http://localhost:" + this.port + CTL.WEB_PATH + "/sys/user/roles/list";
		ResponseEntity<String> entity = new TestRestTemplate().getForEntity(url, String.class);
		assertEquals(HttpStatus.OK, entity.getStatusCode());
		assertTrue("Wrong body (title doesn't match):\n" + entity.getBody(), entity.getBody().contains("<title>用户角色关系表"));
	...
	public void save() throws Exception {
		...
		map.set("id", "root");
		String url = "http://localhost:" + this.port + CTL.WEB_PATH + "/sys/user/roles/save";
